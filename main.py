...
class FeatureComparison(BaseModel):
    ...
    rationale: str = Field(
        ...,
        description=(
            "Rezumat scurt al motivelor cheie (max 4–6 propoziții) care duc la verdictul final. "
            "Enumeră clar criteriile decisive și menționează incertitudinile dacă există."
        ),
    )
...
#Prompturi Generator + Verificator
# =============================================================================

GENERATOR_SYSTEM_PROMPT = """
Ești Generatorul. Trebuie să returnezi STRICT JSON valid conform modelului GeneratorComparison.
Reguli:
- thought: justificare scurtă, bazată pe datele primite. Nu inventa.
- answer: concluzie concisă pentru user.
- confidence: 0..1 (mai mic dacă lipsesc date sau există ambiguități).
- comparison: trebuie să fie un obiect valid conform ComparisonResult (features + verdict).
"""

VERIFIER_SYSTEM_PROMPT = """
Ești Verificatorul. Primești întrebarea + output-ul Generatorului.
Returnezi STRICT JSON conform modelului VerifierReview.
Reguli:
- verdict = 'da' dacă logica și concluzia sunt justificate de date.
- verdict = 'nu' dacă există erori logice / afirmații nejustificate / contradicții.
- verdict = 'nesigur' dacă lipsesc date esențiale pentru decizie.
- Dacă verdict != 'da', oferă feedback_for_retry foarte concret (ce să corecteze).
- Evaluează confidence: este realist? oferă suggested_confidence.
"""
...
async def compară_produse_instructor(
    ...
    # prompt-ul tău existent (poți păstra textul, doar îl mutăm în generator)
    base_user_prompt = f"""Compară aceste produse pentru userul care vrea: "{preferinte}"
...
"""
    ...
    for attempt in range(1, 4):  # max 3 încercări
        user_prompt = base_user_prompt
        if feedback:
            user_prompt += f"\n\nFEEDBACK VERIFICATOR (corectează și reîncearcă):\n{feedback}\n"

        # 1) GENERATOR
        try:
            gen = instructor_client.chat.completions.create(
                model=MODEL,
                messages=[
                    {"role": "system", "content": GENERATOR_SYSTEM_PROMPT},
                    {"role": "user", "content": user_prompt}
                ],
                response_model=GeneratorComparison,
                max_retries=2,   # reîncearcă doar dacă JSON invalid
                temperature=0,
                max_tokens=3000
            )
            last_gen = gen
...
ÎNTREBARE / CERINȚĂ USER:
{preferinte}

DATE PRODUS A (rezumat):
{prod_a.titlu}

DATE PRODUS B (rezumat):
{prod_b.titlu}
...
Verifică dacă:
- concluzia e susținută de date
- nu sunt afirmații inventate
- winner-ul e coerent cu preferințele
- confidence e realist
"""

        try:
            ver = instructor_client.chat.completions.create(
                model=MODEL,
                messages=[
        ...
        # Accept
        if ver.verdict == "da":
            # opțional: poți alinia confidence la suggested_confidence (nu e obligatoriu)
            # gen.confidence = ver.suggested_confidence
            return gen.comparison

        # Retry
        feedback = ver.feedback_for_retry or (
            f"Motiv: {ver.reason}. Ajustează logica și răspunsul. "
            f"Confidence pare: {ver.confidence_assessment} (sugerat {ver.suggested_confidence})."
        )

    # Dacă după 3 încercări tot nu e acceptat:
    # alegem să returnăm ultimul comparison (sau poți arunca HTTPException)
    if last_gen and last_ver:
        return last_gen.comparison

    raise HTTPException(503, "Pipeline failed without outputs.")

#Generator + Verificator (modele)
# =============================================================================

class GeneratorComparison(BaseModel):
    """
    Output-ul Generatorului: oferă GÂNDIRE + RĂSPUNS + confidence
    și include comparația structurată (ComparisonResult).
    """
    thought: str = Field(
        ...,
        description="GÂNDIRE: justificare scurtă (2–6 propoziții), bazată pe date. Nu inventa."
    )
    answer: str = Field(
        ...,
        description="RĂSPUNS: concluzie scurtă pentru user (1–3 propoziții)."
    )
    confidence: float = Field(
        ...,
        ge=0.0,
        le=1.0,
        description="Scor de încredere 0..1. Scade dacă lipsesc date/ambiguități."
    )
    comparison: ComparisonResult = Field(
        ...,
        description="Rezultatul complet structurat conform ComparisonResult."
    )


class VerifierReview(BaseModel):
    """
    Output-ul Verificatorului: acceptă / respinge / nesigur + feedback.
    """
    verdict: str = Field(pattern="^(da|nu|nesigur)$", description="Validare: da/nu/nesigur")
    reason: str = Field(..., description="Motiv scurt pentru verdict.")
    confidence_assessment: str = Field(
        ...,
        description="Analizează dacă confidence-ul Generatorului este realist."
    )
    suggested_confidence: float = Field(
        ...,
        ge=0.0,
        le=1.0,
        description="Confidence recomandat de Verificator."
    )
    feedback_for_retry: Optional[str] = Field(
        default=None,
        description="Feedback pentru reîncercare (doar dacă verdict != 'da')."
    )
# =============================================================================
# FASTAPI APP
# =============================================================================
